...
import ch.maxant.kdc.mf.dsc.entity.ConditionEntity
...
class ConditionsResource(
    ...
    fun validateSyncTime(@PathParam("contractId") contractId: UUID, @PathParam("syncTimestamp") syncTimestamp: Long) =
        Response.ok(ConditionEntity.Queries.countByContractIdAndNotSyncTimestamp(em, contractId, syncTimestamp)).build()

    @GET
    @Path("/{contractId}")
    @Timed(unit = MetricUnits.MILLISECONDS)
    fun getByContractId(@PathParam("contractId") contractId: UUID) =
        Response.ok(ConditionEntity.Queries.findByContractId(em, contractId)).build()
