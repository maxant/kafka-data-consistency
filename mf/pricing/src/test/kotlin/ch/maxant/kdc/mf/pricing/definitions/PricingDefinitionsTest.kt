...
class PricingDefinitionsTest {
    ...
    fun unknownLeafComponent() {
        ...
        assertEquals("no pricing rule found for leaf component UnknownChild", assertThrows<ValidationException> { Prices.findRule(milk)(milk, emptyMap()) }.message)
    }

    @Test
    fun milkNoVolume() {
        val milk = TreeComponent("1", "Milk", emptyList(), emptyList())
        assertEquals("component Milk is missing config for VOLUME", assertThrows<MissingConfigException> { Prices.findRule(milk)(milk, emptyMap()) }.message)
    }

    @Test
    fun milkVolumeWrongUnits() {
        val milk = TreeComponent("1", "Milk", listOf(Configuration("VOLUME", "100", "ASDF")), emptyList())
        assertEquals("VOLUME has unexpected units ASDF instead of MILLILITRES", assertThrows<IllegalArgumentException> { Prices.findRule(milk)(milk, emptyMap()) }.message)
    }

    @Test
    fun milkNoFat() {
        val milk = TreeComponent("1", "Milk", listOf(Configuration("VOLUME", "100", "MILLILITRES")), emptyList())
        assertEquals("component Milk is missing config for FAT_CONTENT", assertThrows<MissingConfigException> { Prices.findRule(milk)(milk, emptyMap()) }.message)
    }
    ...
    fun milkFatContentWrongUnits() {
        ...
        assertEquals("FAT_CONTENT has unexpected units ASDF instead of PERCENT", assertThrows<IllegalArgumentException> { Prices.findRule(milk)(milk, emptyMap()) }.message)
    }
    ...
    fun milkGood() {
        ...
        val price = Prices.findRule(milk)(milk, emptyMap())
        assertTrue(BigDecimal("1.08") <= price.total)
        assertTrue(BigDecimal("1.10") >= price.total)
